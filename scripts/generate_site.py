...
def build_html(items: list[dict], all_items: list[dict], all_window: list[dict], start: datetime, end: datetime, generated_at: datetime, last_signal: datetime | None, fetched_total: int | None) -> str:
    ...
    for n, item in enumerate(display_items, 1):
        ...
        cards.append(f"""
<article class="signal" data-revealed="{'1' if revealed else '0'}" data-high="{'1' if score >= 6 else '0'}" data-rc="{'1' if item.get('residential_complexes') else '0'}" data-dev="{'1' if item.get('developers') else '0'}" data-legal="{'1' if stage in {'суд','гарантия'} else '0'}">
...
if(loadMore){{loadMore.addEventListener('click',()=>{{const hidden=cards.filter(card=>card.dataset.revealed==='0').slice(0, Number(loadMore.dataset.step||24)); hidden.forEach(card=>card.dataset.revealed='1'); applyFilters();}}); updateLoadMore();}}
</script></body></html>"""
