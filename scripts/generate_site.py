...
def process_insight(item: dict, idx: int) -> tuple[list[str], str]:
    ...
    if counts and max(counts) >= 60:
        return ["#много_замечаний", "#ручная_координация"], f"В {subject} число замечаний уже само становится управленческой задачей: {max(counts)} пунктов нужно разложить по зонам, подрядчикам и повторным осмотрам. Риск не в том, что дефекты есть, а в том, что часть пунктов закроется словами, без проверки на месте."
    if counts and max(counts) >= 20:
        return ["#очередь_дефектов", "#повторная_приёмка"], f"{max(counts)} дефектов по {subject} - это не один спорный пункт, а очередь работ. Здесь нужен видимый маршрут: что исправляют сразу, что уходит подрядчику, что проверяется повторно и что остается открытым после второго осмотра."
    if stage == "суд":
        return ["#судебный_след", "#доказательная_база"], f"У {subject} сигнал уже дошел до спорной плоскости. Тут ценность не в громкости поста, а в восстановлении картины: что именно фиксировали, какие материалы остались, где возникло расхождение и чем подтверждали позицию сторон."
    if stage == "гарантия":
        return ["#гарантийный_хвост", "#память_замечания"], f"В {subject} важен момент после передачи квартиры. Если замечание возвращается в гарантию, системе нужна память: где его нашли, как закрывали, кто отвечал и почему дефект снова появился в повестке."
    if stage == "выдача ключей":
        return ["#выдача_ключей", "#поток_замечаний"], f"На выдаче у {subject} проблема может быть не в одном дефекте, а в потоке. Одновременно идут осмотры, обещания, переносы и повторные визиты. Если статусы живут в чатах, менеджер быстро теряет, что уже закрыто, а что просто пообещали закрыть."
    if "вентиляция" in cats or "сантехника/санузлы" in cats:
        return ["#инженерный_риск", "#повторная_проверка"], f"В {subject} инженерный блок нельзя закрывать как обычную косметику. По таким пунктам важно видеть, что проверяли, чем подтверждали исправление и кто принял результат. Иначе запись есть, а уверенности, что проблема ушла, нет."
    ...
    if "окна/стеклопакеты/откосы" in cats:
        return ["#оконный_узел", "#повторяемость_подрядчика"], f"В {subject} оконный узел выглядит как набор мелких пунктов, но на потоке это уже проверка подрядчика. Если похожие замечания повторяются по профилю, откосам или герметизации, важно видеть не только квартиру, а серию по объекту."
    if "смета/объемы/договор/скрытые работы" in cats:
        return ["#скрытые_работы", "#контроль_подрядчика"], f"У {subject} сигнал уходит глубже обычной приемки: появляется подрядчик, скрытые работы или объемы. Здесь недостаточно фото дефекта. Нужна связка с предписанием, сроком, ответственным и подтверждением, что работа реально переделана."
    if channel in ENRICHMENT_CHANNELS:
