...
function weekIsActive(){{const section=document.getElementById('week');const r=section.getBoundingClientRect();return r.top<innerHeight*.75&&r.bottom>innerHeight*.18;}}
function keepWeekAnchor(beforeTop){{if(beforeTop===null)return;requestAnimationFrame(()=>{{const after=document.getElementById('week').getBoundingClientRect().top;scrollBy(0,after-beforeTop);}});}}
...
function renderPeriod(anchorWeek=false){{const beforeTop=anchorWeek?document.getElementById('week').getBoundingClientRect().top:null;const items=periodItems();periodText.textContent=fmtDate(dateFromEl.value)+' — '+fmtDate(dateToEl.value);statSignals.textContent=items.length;statRc.textContent=items.filter(x=>x.rc_count).length;statHigh.textContent=items.filter(x=>x.high).length;funnelPeriod.textContent=items.length;const stages=countRows(items,x=>x.stage);weekRows.innerHTML=stages.map(([stage,n])=>{{const examples=items.filter(x=>x.stage===stage).slice(0,4).map(x=>`<a href="${{esc(x.link)}}" target="_blank" rel="noreferrer">${{esc(x.title)}}</a>`).join('; ');return `<tr><td>${{esc(stage)}}</td><td>${{n}}</td><td>${{items.length?Math.round(n/items.length*100):0}}%</td><td>${{examples}}</td></tr>`}}).join('')||'<tr><td colspan="4">Нет карточек за выбранный период</td></tr>';const cats=countRows(items,x=>x.categories).slice(0,12);painRows.innerHTML=cats.map(([cat,n])=>`<tr><td>${{esc(cat)}}</td><td>${{n}}</td></tr>`).join('')||'<tr><td colspan="2">Нет категорий за выбранный период</td></tr>';catalogRows.innerHTML=items.map(x=>`<tr><td>#${{String(x.n).padStart(3,'0')}}</td><td>${{esc(x.date_text)}}</td><td>${{esc(x.title)}}</td><td>${{esc(x.developers)}}</td><td>${{esc(x.stage)}}</td><td>${{esc(x.label)}}</td><td><a href="${{esc(x.link)}}" target="_blank" rel="noreferrer">источник</a></td></tr>`).join('')||'<tr><td colspan="7">Нет карточек за выбранный период</td></tr>';cards.forEach(card=>{{card.dataset.inPeriod=inRangeDate(card.dataset.date)?'1':'0';card.dataset.revealed='0';}});cards.filter(card=>card.dataset.inPeriod==='1').slice(0,24).forEach(card=>card.dataset.revealed='1');applyFilters();keepWeekAnchor(beforeTop);}}
function setPreset(range){{const anchor=weekIsActive();document.querySelectorAll('[data-range]').forEach(b=>b.classList.toggle('active',b.dataset.range===range));if(range==='all'){{dateFromEl.value=minDate;dateToEl.value=maxDate;}}else{{dateToEl.value=maxDate;dateFromEl.value=addDays(maxDate,-Number(range)+1);}}renderPeriod(anchor);}}
...
document.querySelectorAll('[data-range]').forEach(b=>b.addEventListener('click',()=>setPreset(b.dataset.range))); [dateFromEl,dateToEl].forEach(x=>x.addEventListener('input',()=>renderPeriod(weekIsActive())));
