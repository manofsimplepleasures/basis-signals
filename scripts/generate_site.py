...
</style></head><body id="top"><a href="#top" class="top-float" aria-label="Вернуться наверх">↑ наверх</a><div class="wrap"><header class="shell"><div class="titlebar"><strong>basis_signal.exe</strong><span>public · open sources</span></div><div class="hero"><p class="small">{update_text}</p><h1>Базис - Сигналы</h1><p class="lead">Публичные наблюдения из открытых источников. Каждая карточка ведёт к первоисточнику.</p><div class="funnel" aria-label="Воронка отбора"><div class="step"><b>{viewed_total}</b>публикаций просмотрено</div><div class="step"><b>{len(all_items)}</b>распознано</div><div class="step"><b id="funnelPeriod">{len(all_window)}</b>сигналов за период</div></div></div></header><nav><a href="#radar">Радар сегодня</a><a href="#week">Сюжеты</a><a href="#catalog">Картотека ЖК</a><a href="#painmap">Карта болей</a><a href="#sources">Источники</a></nav><div class="layout"><aside><div class="panel"><p><b>ФИЛЬТР</b></p><label><input type="checkbox" id="fHigh"> больше всего конкретики</label><label><input type="checkbox" id="fRc"> есть ЖК</label><label><input type="checkbox" id="fDev"> есть застройщик</label><label><input type="checkbox" id="fLegal"> гарантия / суд</label><button class="btn" id="reset">сбросить</button><div id="filterStatus" class="status">Фильтр применяется сразу · показано {len(items)} из {len(display_items)}</div></div><div class="panel"><p><b>ПЕРИОД ДАННЫХ</b></p><div class="period-controls"><label>С даты<input id="dateFrom" type="date"></label><label>По дату<input id="dateTo" type="date"></label><div class="period-buttons"><button type="button" data-range="7">неделя</button><button type="button" data-range="30">месяц</button><button type="button" data-range="all">весь период</button></div></div><p id="periodText">{period}</p><p class="small">Последний найденный сигнал: {last_signal_text}</p></div><div class="panel"><p><b>КАНАЛЫ</b></p><p>{channel_links(channels)}</p></div></aside><main><section id="radar" class="panel"><h2>Радар сегодня <a href="#top">↑ наверх</a></h2><p>Последние отобранные сигналы за 14 дней. На главной не все найденные посты, а карточки, где есть больше всего конкретики.</p><div class="grid">{cards_html}</div><button class="btn" id="loadMore" data-step="24">показать ещё · сейчас {len(items)} из {len(display_items)}</button></section><section id="week" class="panel"><h2>Сюжеты <a href="#top">↑ наверх</a></h2><p class="small">Выберите неделю, месяц или любой диапазон: таблица покажет, в какой стадии сколько карточек за этот период.</p><table><thead><tr><th>Стадия</th><th>Карточек</th><th>Доля</th><th>Примеры</th></tr></thead><tbody id="weekRows">{stage_rows}</tbody></table></section><section id="catalog" class="panel"><h2>Картотека ЖК <a href="#top">↑ наверх</a></h2><p class="small">ЖК сгруппированы по застройщику и отсортированы по количеству упоминаний за выбранный период. Так видно, где повторяются объекты и какие девелоперы чаще попадают в сигналы.</p><table><thead><tr><th>Застройщик</th><th>Упоминаний</th><th>ЖК</th><th>Даты</th><th>Примеры сигналов</th></tr></thead><tbody id="catalogRows">{catalog_rows}</tbody></table></section><section id="painmap" class="panel"><h2>Карта болей <a href="#top">↑ наверх</a></h2><p class="small">Это не список объектов, а карта повторяющихся тем дефектов за выбранный период: какие категории чаще встречаются в карточках и какую долю периода занимают.</p><table><thead><tr><th>Категория боли</th><th>Сигналов</th><th>Доля периода</th><th>Примеры</th></tr></thead><tbody id="painRows">{cat_rows}</tbody></table></section><section id="sources" class="panel"><h2>Источники <a href="#top">↑ наверх</a></h2><p>Используются публичные Telegram web previews. Каналы приёмки и технадзора отделяются от рыночного обогащения.</p><p>{channel_links(channels)}</p></section><footer class="panel"><p class="small">Данные обновляются ежедневным прогоном: сайт хранит последний JSON и сжатый архив по дням. Сырые HTML-страницы и медиа не хранятся.</p></footer></main></div></div><script>
...
const dates=SIGNAL_DATA.map(x=>x.date).filter(Boolean).sort(); const minDate=dates[0]||''; const maxDate=dates[dates.length-1]||'';
