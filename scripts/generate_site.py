...
def build_html(items: list[dict], all_items: list[dict], all_window: list[dict], start: datetime, end: datetime, generated_at: datetime, last_signal: datetime | None, fetched_total: int | None) -> str:
    ...
    for n, item in enumerate(display_items, 1):
        ...
        cards.append(f"""
<article class="signal" data-revealed="{'1' if revealed else '0'}" data-in-period="1" data-date="{html.escape(date_iso)}" data-stage="{html.escape(stage)}" data-high="{'1' if score >= 6 else '0'}" data-rc="{'1' if item.get('residential_complexes') else '0'}" data-dev="{'1' if item.get('developers') else '0'}" data-legal="{'1' if stage in {'суд','гарантия'} else '0'}">
  <div class="cardtop"><b>СИГНАЛ #{n:03d}</b><span class="{weak_class.strip()}">{label}</span></div>
  <h3>{html.escape(title)}</h3>
  <p class="source"><a href="{html.escape(link)}" target="_blank" rel="noreferrer">@{html.escape(ch)}</a> · {date_s} · {source_type(ch)}</p>
  <div class="meta"><span class="{weak_class.strip()}">{html.escape(stage_label.upper())}</span><span>ЖК: {html.escape(rc)}</span><span>Застройщик: {html.escape(dev)}</span><span>Замечаний: {html.escape(', '.join(map(str, counts)) or 'не указано')}</span></div>
...
""")
        rows.append(f"<tr data-date=\"{html.escape(date_iso)}\" data-stage=\"{html.escape(stage)}\"><td>#{n:03d}</td><td>{date_s}</td><td>{html.escape(title)}</td><td>{html.escape(dev)}</td><td>{html.escape(stage)}</td><td>{html.escape(label)}</td><td><a href=\"{html.escape(link)}\" target=\"_blank\" rel=\"noreferrer\">источник</a></td></tr>")
    ...
    for n, item in enumerate(display_items, 1):
        ...
        client_items.append({
            ...
            "title": item_title(item),
            "residential_complexes": item.get("residential_complexes") or [],
            "developers": ", ".join(item.get("developers") or []) or "не указан",
            "stage": display_stage(stage),
            "label": concrete_label(score_item(item)),
            "categories": item.get("categories") or [],
            "rc_count": 1 if item.get("residential_complexes") else 0,
...
const ANCHOR_SECTIONS=['week','catalog','painmap'];
...
function renderPeriod(anchorId=null){{const anchor=anchorId?{{id:anchorId,top:document.getElementById(anchorId).getBoundingClientRect().top}}:null;const items=periodItems();periodText.textContent=fmtDate(dateFromEl.value)+' — '+fmtDate(dateToEl.value);funnelPeriod.textContent=items.length;const stages=countRows(items,x=>x.stage);weekRows.innerHTML=stages.map(([stage,n])=>{{const examples=items.filter(x=>x.stage===stage).slice(0,4).map(x=>`<a href="${{esc(x.link)}}" target="_blank" rel="noreferrer">${{esc(x.title)}}</a>`).join('; ');return `<tr><td>${{esc(stage)}}</td><td>${{n}}</td><td>${{items.length?Math.round(n/items.length*100):0}}%</td><td>${{examples}}</td></tr>`}}).join('')||'<tr><td colspan="4">Нет карточек за выбранный период</td></tr>';const cats=countRows(items,x=>x.categories).slice(0,12);painRows.innerHTML=cats.map(([cat,n])=>{{const examples=items.filter(x=>(x.categories||[]).includes(cat)).slice(0,3).map(x=>`<a href="${{esc(x.link)}}" target="_blank" rel="noreferrer">${{esc(x.title)}}</a>`).join('; ');return `<tr><td>${{esc(cat)}}</td><td>${{n}}</td><td>${{items.length?Math.round(n/items.length*100):0}}%</td><td>${{examples}}</td></tr>`}}).join('')||'<tr><td colspan="4">Нет категорий за выбранный период</td></tr>';const devMap=new Map();items.forEach(x=>{{const devs=(x.developers||'не указан').split(',').map(s=>s.trim()).filter(Boolean);const rcs=(x.residential_complexes||[]).length?x.residential_complexes:['ЖК не указан'];devs.forEach(dev=>{{if(!devMap.has(dev))devMap.set(dev,{{count:0,rc:new Map(),dates:new Set(),examples:[]}});const row=devMap.get(dev);row.count++;row.dates.add(x.date_text);rcs.forEach(rc=>row.rc.set(rc,(row.rc.get(rc)||0)+1));if(row.examples.length<3)row.examples.push(`<a href="${{esc(x.link)}}" target="_blank" rel="noreferrer">${{esc(x.title)}}</a>`);}});}});catalogRows.innerHTML=[...devMap.entries()].sort((a,b)=>b[1].count-a[1].count||a[0].localeCompare(b[0])).map(([dev,row])=>{{const rc=[...row.rc.entries()].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0])).slice(0,8).map(([name,n])=>`${{esc(name)}} <span class="small">×${{n}}</span>`).join('<br>');const dates=[...row.dates].slice(0,6).join(', ');return `<tr><td>${{esc(dev)}}</td><td>${{row.count}}</td><td>${{rc}}</td><td>${{esc(dates)}}</td><td>${{row.examples.join('; ')}}</td></tr>`}}).join('')||'<tr><td colspan="5">Нет ЖК за выбранный период</td></tr>';cards.forEach(card=>{{card.dataset.inPeriod=inRangeDate(card.dataset.date)?'1':'0';card.dataset.revealed='0';}});cards.filter(card=>card.dataset.inPeriod==='1').slice(0,24).forEach(card=>card.dataset.revealed='1');applyFilters();keepSectionAnchor(anchor);}}
